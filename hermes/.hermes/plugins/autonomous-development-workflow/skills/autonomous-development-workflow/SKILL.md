...
Use this skill only for Kanban cards that already have an `autonomous-development.v1` or `direct-implementation.v1` Manifest. Natural-language labels in the card body are not an enablement signal.
...
3. Exercise real browser, Obsidian, CLI, or application paths only after the controller returns `acceptance_required`. Deterministic tests, lint, or code review are not product acceptance. `direct-implementation.v1` never returns `acceptance_required`; keep advancing until the controller completes or blocks.
