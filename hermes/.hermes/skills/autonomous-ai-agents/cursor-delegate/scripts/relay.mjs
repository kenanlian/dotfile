 ...
 *
 * Dispatch a self-contained brief to the Cursor Agent CLI (`agent -p`),
 ...
 * orchestrator-agnostic. Verified against agent 2026.07.23 on Windows.
 *
 * Trust posture: relay.mjs itself makes no network calls, reads or writes no
 * credentials, and sends no telemetry; it has no dependencies (Node built-ins
 * only). It shells out only to `agent` and `git` (plus taskkill on
 * Windows for process-tree termination). The `agent` process it
 ...
 *
 * Autonomy (local Hermes hardening): a fresh run defaults to Cursor plan mode
 * (read-only/planning, no edits). Pass `--force` explicitly to enable writes
 * and automatic command approval. `--no-force` is intentionally rejected.
 ...
 *                           auto). List names with `agent models`.
 *   --read-only             Run in Cursor's plan mode (the local default).
 *   --force                 Explicitly enable writes and automatic command
 *                           approval for this run.
 *   --sandbox <mode>        Explicitly enable or disable Cursor's sandbox for
 *                           this dispatch (enabled | disabled).
 *   --no-force              Unsupported by this hardened local variant; use
 *                           --read-only or --force.
...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i += 1) {
    ...
    switch (arg) {
      ...
      case "--read-only": opts.readOnly = true; opts.force = false; flagged.add("readOnly"); flagged.add("force"); break;
      case "--force": opts.readOnly = false; opts.force = true; flagged.add("readOnly"); flagged.add("force"); break;
      case "--sandbox": opts.sandbox = next(); flagged.add("sandbox"); break;
      case "--no-force": fail("--no-force is disabled by local hardening; use --read-only or explicit --force"); break;
  ...
  // agent resolves a relative --add-dir against ITS cwd, so resolve
...
function headerComment() {
  ...
  if (!match) return "relay.mjs — dispatch a brief to agent -p\n";
...
function winq(value) {
  // shell:true on win32 (needed for the agent.cmd shim) doesn't quote
...
function reportUnavailable(writeResult, resultPath) {
  ...
  process.stderr.write("relay: `agent` not found on PATH. Install the Cursor CLI (https://cursor.com/cli) and run `agent login`.\n");
  process.exit(127);
}
...
function reportVersionFailure(opts, writeResult, run, error, timeoutMs) {
  ...
  const message = timedOut
    ? `agent --version preflight timed out after ${Math.min(timeoutMs, VERSION_PROBE_TIMEOUT_MS)}ms; Cursor was not dispatched`
    : `agent --version preflight failed${Number.isInteger(error?.status) ? ` with exit ${error.status}` : ""}; Cursor was not dispatched`;
  const result = writeResult({
  ...
  for (const sig of ["SIGTERM", "SIGINT", "SIGHUP"]) {
    const handler = () => {
      ...
      const result = writeResult({
        ...
        finalMessage: "",
        touchedFiles: gitTouchedFiles(opts.cd),
        error: `the relay was killed by ${sig} during the agent version preflight; Cursor was not dispatched`,
      });
      printSummary(result, run.resultPath);
      process.exit(result.exitCode);
    };
...
function dispatchToCursor(opts, brief, run, writeResult) {
  // A shell launch on Windows so the agent.cmd shim resolves (see
  ...
  const scan = makeEventScanner((event) => {
    ...
    // Current Cursor plan mode may put the actual deliverable only in a
    // createPlan tool call while the final result event contains progress text.
    // Capture the newest plan so read-only delegation returns the real report.
    const plan = event?.tool_call?.createPlanToolCall?.args?.plan;
    if (typeof plan === "string" && plan.trim()) planMessage = plan;
  ...
  const assembleFinal = () => {
    // In Cursor plan mode the deliverable may exist only in createPlan args;
    // otherwise prefer the result event and fall back to assistant text.
    const message = opts.readOnly && planMessage && planMessage.trim()
      ? planMessage
  ...
  // no result.json and leaves the agent child running or dying mid-edit with
  ...
  for (const sig of ["SIGTERM", "SIGINT", "SIGHUP"]) {
    process.on(sig, () => {
      ...
      const abortedFields = {
        ...
        error: `the relay was killed by ${sig}; agent was terminated with it — inspect the working tree before re-dispatching`,
      };
  ...
  child.on("close", (code, signal) => {
    ...
    // A timed-out run is failed even if agent handles SIGTERM by exiting 0 —
    ...
    const result = writeResult({
      ...
      ...(watchdogFired ? { error: `agent did not finish within --timeout ${opts.timeout}; killed by the relay watchdog` } : {}),
      ...(resultIsError && !watchdogFired ? { error: "agent reported an error result (is_error: true in its result event)" } : {}),
    });
    printSummary(result, run.resultPath);
    process.exit(result.exitCode);
  });
...
async function main() {
  ...
  // Keep the CLI availability probe bounded even when the task watchdog is off.
...
function printSummary(result, resultPath) {
  const lines = [];
  lines.push("");
  lines.push(`relay: ${result.status} (exit ${result.exitCode}${result.signal ? `, killed by ${result.signal}` : ""})  ·  agent ${result.cursorAgentVersion ?? "?"}`);
  if (result.signal === "SIGKILL" && result.status === "failed") lines.push("hint: the host killed the process (commonly the OOM killer or a supervisor timeout) — this is not a agent error; check host memory and re-dispatch, or split the task into smaller briefs.");
  if (result.signal === "SIGTERM" && result.status === "failed") lines.push("hint: something outside the relay terminated agent (a supervisor, the session ending, or a manual kill) — when the relay itself does the killing it reports status \"timeout\" or \"aborted\" instead; inspect the working tree before re-dispatching.");
