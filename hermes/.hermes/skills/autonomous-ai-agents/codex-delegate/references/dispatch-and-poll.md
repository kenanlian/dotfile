...
| `--sandbox <mode>` | `read-only` \| `workspace-write` \| `danger-full-access` (local default: `read-only`; writing requires explicit `workspace-write`). |
...
| `--timeout <dur>` | Optional Relay-side watchdog (e.g. `30m`, `4h`); off by default. Normal orchestration omits it; if an external hard bound is required, prefer a deliberately long safety guard rather than estimating task duration. On expiry the child is killed and `result.json` gets `status: "timeout"`. |
